...
GITUSER="gaoweifan"
REPONAME="quectel-rgmii-toolkit-cn"
GITTREE="SDXLEMUR"
GITMAINTREE="SDXLEMUR"
GITDEVTREE="dev-SDXLEMUR-cn"
...
if echo "$ARCH" | grep -q "aarch64"; then
    cd /tmp && wget -O RM55x_rcPCIe_toolkit.sh https://gh-proxy.com/https://raw.githubusercontent.com/gaoweifan/quectel-rgmii-toolkit-cn/SDXPINN/RM55x_rcPCIe_toolkit.sh && chmod +x RM55x_rcPCIe_toolkit.sh && ./RM55x_rcPCIe_toolkit.sh && cd /
    exit 0
    ...
    case $choice in
		...
		12) 
			...
			mkdir /usrdata/root
     	    mkdir /usrdata/root/bin
			cd /usrdata/root/bin
     	    wget -O fast https://gh-proxy.com/https://github.com/ddo/fast/releases/download/v0.0.4/fast_linux_arm && chmod +x fast
     	    cd /
